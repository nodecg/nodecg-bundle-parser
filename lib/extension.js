...
module.exports = function(bundleDir) {
    ...
    if (directoryExists && !fs.lstatSync(directoryPath).isDirectory()) {
        throw new Error('Bundle %s has an illegal file named "extension" in its root. ' +
    ...
    if (singleFileExists && directoryExists) {
        throw new Error('Bundle %s has both "extension.js" and a folder named "extension". ' +
