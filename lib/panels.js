...
module.exports = function (dashboardDir, manifest) {
	...
	manifest.dashboardPanels.forEach((panel, index) => {
		const missingProps = [];
		...
		if (dupeFound) {
			throw new Error(`Panel #${index} (${panel.name}) has the same name as another panel in ${manifest.name}.`);
		}
		...
		if (!fs.existsSync(filePath)) {
			throw new Error(`Panel file "${panel.file}" in bundle "${manifest.name}" does not exist.`);
		}
		...
		if ($('head').length < 1) {
			throw new Error(`Panel "${path.basename(panel.file)}" in bundle "${manifest.name}" has no <head>, ` +
		...
		if (!html.match(/(<!doctype )/ig)) {
			throw new Error(`Panel "${path.basename(panel.file)}" in bundle "${manifest.name}" has no DOCTYPE,` +
