...
module.exports = function (manifestPath) {
	...
	const bundleFolderName = path.dirname(manifestPath).split(path.sep).pop();
	if (bundleFolderName !== manifest.name) {
		throw new Error(`${manifest.name}'s folder is named "${bundleFolderName}".` +
			`Please rename it to "${manifest.name}".`);
	}

	const obj = manifest.nodecg;

	// Ensure that some key properties are not undefined.
	obj.uploads = obj.uploads || {};
	obj.uploads.enabled = Boolean(obj.uploads.enabled); // No undefined please
	obj.uploads.allowedTypes = obj.uploads.allowedTypes || [];
